"""Agendamento diário de leitura do Gmail e da Google Agenda."""
...
USUARIO = os.environ.get('GOOGLE_SYNC_USERNAME', '')
HORA = int(os.environ.get('GOOGLE_SYNC_SCHEDULE_HOUR', '7'))
MINUTO = int(os.environ.get('GOOGLE_SYNC_SCHEDULE_MINUTE', '25'))
ATRASO = int(os.environ.get('GOOGLE_SYNC_RETRY_DELAY_MINUTES', '15'))
...
def main():
    if not USUARIO:
        raise SystemExit('GOOGLE_SYNC_USERNAME não configurado.')
    while True:
        alvo = proxima_execucao()
        print(f'Próxima sincronização Google: {alvo.isoformat()}', flush=True)
